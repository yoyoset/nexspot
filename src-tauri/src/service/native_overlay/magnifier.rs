...
pub fn draw_magnifier(
    ...
    // 1. Calculate magnifier position (follow mouse)
    let mag_size = 120; // 120x120 pixels
    let zoom_factor = 2;
    let offset_x = 20;
    let offset_y = 20;
    ...
    let brush_bg = win32::gdi::create_solid_brush(0x202020)?; // Dark grey background
    win32::gdi::fill_rect(hdc_mem, &rect, &brush_bg);

    let brush_border = win32::gdi::create_solid_brush(0xFFFFFF)?; // White border
