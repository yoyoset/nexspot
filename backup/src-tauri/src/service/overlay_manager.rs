...
impl OverlayManager {
    ...
    fn create_window(
        ...
        */
        ...
        // FIX: Force click-through for P8 Native Input debugging
        // ensure input falls through to the Native Window behind it.
        win.set_ignore_cursor_events(true).unwrap_or_default();
            ...
            if win.label().starts_with("overlay-") {
                if let Ok(hwnd) = win.hwnd() {
                    unsafe {
                        ...
                        crate::service::logger::log(
                            "OverlayManager",
                            &"WebView shown, set to TOP Z-order".to_string(),
                        );
                    }
                } else {
                    win.show().unwrap();
    ...
    pub fn set_click_through(app: &AppHandle, enabled: bool) {
        for win in app.webview_windows().values() {
            if win.label().starts_with("overlay-") {
                // enabled = true means CLICK THROUGH (ignore events)
                // enabled = false means INTERACTIVE (accept events)
                let _ = win.set_ignore_cursor_events(enabled);
                crate::service::logger::log(
                    "OverlayManager",
                    &format!("Web ClickThrough: {}", enabled),
                );
            }
        }
    }
