...
- 定位：PIN 合集 = 可视化的临时多剪贴板合集（OCR 全文也会静默入集）。
...
> 2026-06-04 产品方向定为"专注截图工具"，AI 子系统（后端 `service/ai/`、前端 `AgentCoreTab.tsx`、相关 locale）已整体删除，详见 [08-AUDIT B3](08-AUDIT.md)。
> 历史规格（截图+提示词→AI→PIN 卡片回流）保留在归档 `TOOLS.md` 中，若未来重启 AI 方向可参考。
